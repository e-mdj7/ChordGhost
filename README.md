...
- **QML** (Qt Modeling Language) with `QtQuick`, `QtQuick.Controls`, and `QtQuick.Layouts` — the UI/scripting language MuseScore 4 plugins are written in (thanks youtube).
- Targets the **MuseScore 4 Plugin API** (`import MuseScore 4.X`).
- Single-file plugin, no build step and no external dependencies, UI, music theory logic, and staff rendering lives in `ChordGhost.qml`.
...
- The staff view (center) renders the chord across grand staff, plus a side-by-side comparison of all inversions with figured bass (6, 6/4, 7, 6/5, 4/3, 2) (this servers as a visualization when voicing for orchestra - although not compatible with transposition (for now)).
...
- Reads the current selection from the open MuseScore score, detects the chord in each measure (best-fit match against ~25 chord qualities), and labels it with a Roman numeral to look cool.
- Auto-detects major vs. relative-minor mode from the chord roots present.
- **Cadence detection**: flags Authentic (V→I), Plagal (IV→I), Deceptive (V→vi), Half (→V), and full ii/IV→V→I progressions in the analyzed sequence -- please note that this is only based on a bit of math and musical theory, it will not replace what your heart desires to put on the sheets.
- **Mood detection**: scores the sequence against 8 harmonic "mood" fingerprints (Luminoso, Melancólico, Tensión, Épico, Onírico, Oscuro, Nostálgico, Triunfante) based on major/minor/diminished chord density -- sorry, spanish only for now.
...
3. In MuseScore: **Plugins → Plugin Manager**, enable "Chord Ghost", then restart MuseScore if prompted -- PLEASE REMEMBER TO MAP THE PLUGIN TO A KEY/KEYS (mine is Alt+G).
